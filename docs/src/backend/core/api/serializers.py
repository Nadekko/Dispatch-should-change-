...
class ServerCreateDocumentSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        parent_document_id = validated_data.get("parent_document_id")
        if parent_document_id is not None:
            try:
                parent_document = models.Document.objects.get(
            ...
            if user is not None:
                if not parent_document.get_abilities(user).get("children_create"):
                    raise serializers.ValidationError(
                        {
                            "parent_document_id": [
                                "User cannot create children under this parent document."
                            ]
                        }
                    )
            elif not models.Invitation.objects.filter(
                ...
                raise serializers.ValidationError(
                    {
                        "parent_document_id": [
                            "Invitation owner does not match the parent document."
                        ]
                    }
                )
        ...
        document = create_tree_node_with_retry(
            lambda: (
                parent_document.add_child(
                    title=validated_data["title"],
                    creator=user,
                )
                if parent_document is not None
                else models.Document.add_root(
                    title=validated_data["title"],
                    creator=user,
                )
            )
        ...
        doc_created_properties = {}
        if parent_document is not None:
            doc_created_properties["document_parent"] = str(parent_document.id)
