...
@override_settings(SERVER_TO_SERVER_API_TOKENS=["DummyToken"])
def test_api_documents_create_for_owner_as_child_document(mock_convert_md):
    """Server-to-server creation should support attaching the new document as a child."""
    user = factories.UserFactory(language="en-us")
    parent_document = factories.DocumentFactory(users=[(user, "owner")])
    ...
    assert document.title == "Summary"
    assert document.content == "Converted document content"
    assert document.creator == user
    assert document.accesses.filter(user=user, role="owner").exists()
    mock_convert_md.assert_called_once_with(
        "Summary content", mime_types.MARKDOWN, mime_types.YJS
    )
...
@override_settings(SERVER_TO_SERVER_API_TOKENS=["DummyToken"])
def test_api_documents_create_for_owner_rejects_unknown_parent_document():
    """An explicit parent document must exist before creating a child document."""
    response = APIClient().post(
        "/api/v1.0/documents/create-for-owner/",
        {
            "title": "My Document",
            "content": "Document content",
            "parent_document_id": str(uuid4()),
            "sub": "123",
            "email": "john.doe@example.com",
        },
        format="json",
    ...
    assert response.status_code == 400
    assert response.json() == {
        "parent_document_id": ["Parent document does not exist."]
    }
