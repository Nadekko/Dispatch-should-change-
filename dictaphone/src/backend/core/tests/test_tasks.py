...
@patch("core.tasks.file.session.post")
@patch("core.tasks.file.session.get")
def test_task_handle_transcript_received_links_summary_to_transcript(
    mock_get, mock_post
):
    """Summary jobs should keep a durable link to their source transcript job."""
    ai_transcript_job = factories.AiFileJobFactory(
        type=AiJobTypeChoices.TRANSCRIPT,
        status=AiJobStatusChoices.PENDING,
    )
    ...
    ai_summary_job = AiFileJob.objects.get(remote_job_id="remote-summary-job-id")
    assert ai_summary_job.source_ai_job_id == ai_transcript_job.id
...
@patch("core.tasks.file.session.post")
@patch("core.tasks.file.AiFileJob.to_markdown")
def test_task_create_document_in_docs_creates_summary_as_child_document(
    mock_to_markdown, mock_post, settings
):
    """Summary jobs should be created as Docs children of their transcript document."""
    settings.DOCS_BASE_URL = "https://docs.example.com"
    settings.DOCS_SERVER_TO_SERVER_API_KEY = "docs-api-key"
    ai_transcript_job = factories.AiFileJobFactory(
        type=AiJobTypeChoices.TRANSCRIPT,
        status=AiJobStatusChoices.SUCCESS,
        docs_app_id="transcript-doc-id",
        file__title="Meeting notes",
    )
    ai_summary_job = factories.AiFileJobFactory(
        file=ai_transcript_job.file,
    ...
    mock_to_markdown.return_value = "# Résumé"
    response = Mock()
    response.raise_for_status.return_value = None
    response.status_code = 201
    response.json.return_value = {"id": "summary-doc-id"}
    mock_post.return_value = response
    ...
    assert args == ("https://docs.example.com/api/v1.0/documents/create-for-owner/",)
    assert kwargs["json"]["content"] == "# Résumé"
    assert kwargs["json"]["email"] == ai_summary_job.file.creator.email
    assert kwargs["json"]["sub"] == ai_summary_job.file.creator.sub
    assert kwargs["json"]["send_notification_email"] is True
    assert kwargs["json"]["parent_document_id"] == "transcript-doc-id"
    assert str(kwargs["json"]["title"]) == "Meeting notes - Summary"
    assert kwargs["headers"]["Authorization"].endswith("docs-api-key")
    assert kwargs["timeout"] == (20, 3 * 60)
    ai_summary_job.refresh_from_db()
    assert ai_summary_job.docs_app_id == "summary-doc-id"
